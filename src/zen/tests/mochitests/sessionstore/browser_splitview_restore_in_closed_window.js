...
add_task(async function test_restoreSplitViewInClosedWindow() {
  ...
  Assert.equal(
    ...
    2,
    "A splitview is present before closing the window."
  );
  ...
  const restoredWin = SessionStore.undoCloseWindow(0);
  await promiseDelayedStartupFinished(restoredWin);

  Assert.equal(
    restoredWin.gBrowser.tabs.length,
    2,
    "There should be 2 tabs restored,"
  );
  Assert.equal(
    restoredWin.gBrowser.tabs[1].splitview.tabs.length,
    2,
    "Both tabs in the splitview have been restored to the splitview."
  );
  Assert.equal(
    restoredWin.gBrowser.tabs[1].splitview.splitViewId,
    splitViewId,
    "The correct splitview id is assigned."
  );
...
add_task(
  async function test_restoreClosedTabGroupWithSplitViewFromClosedWindow() {
    let win = await promiseNewWindowLoaded();
    let aboutMozillaTab = BrowserTestUtils.addTab(
      win.gBrowser,
      "about:mozilla"
    );
    ...
    Assert.equal(
      ...
      1,
      "there should be a closed tab group"
    );
    Assert.equal(
