...
ZenGlobalShortcuts::ZenGlobalShortcuts() : mNextInternalId(1) {
...
bool ZenGlobalShortcuts::FindById(uint32_t aInternalId,
                                  nsCString& aOutId) const {
  for (const auto& reg : mRegistrations) {
    if (reg.internalId == aInternalId) {
      aOutId = reg.id;
      return true;
    }
  }
...
void ZenGlobalShortcuts::OnNativeShortcut(uint32_t aInternalId) {
  ...
  nsCString id;
  if (!sInstance->FindById(aInternalId, id)) return;
  DispatchEvent(id);
}

// static
void ZenGlobalShortcuts::DispatchEvent(const nsACString& aId) {
  MOZ_ASSERT(NS_IsMainThread());
  ...
  nsContentUtils::DispatchTrustedEvent(
      doc, nsGlobalWindowOuter::Cast(outer), eventName,
