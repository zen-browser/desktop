...
var gZenWorkspaces = new (class extends nsZenMultiWindowFeature {
  ...
  async groupTabsBySimilarity() {
    ...
    try {
      const { groupTabsBySimilarity } = await this.#getTabsTidyModule();
      await groupTabsBySimilarity({
        window,
        gBrowser,
        gZenUIManager,
        workspaceId,
        unpinnedTabs,
        setGroupingInProgress: (isInProgress) => {
          this._groupingInProgress = isInProgress;
        },
      });
    } catch (error) {
      this._groupingInProgress = false;
      window.dispatchEvent(new CustomEvent('ZenGroupingTabsEnd'));
      console.error('gZenWorkspaces: Failed to load ZenTabsTidy.sys.mjs', error);
    }
  }
