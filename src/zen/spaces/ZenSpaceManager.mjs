...
class nsZenWorkspaces {
  ...
  async _animateTabs(
    ...
    if (this.shouldAnimateEssentials && previousWorkspace) {
      ...
      const essentialsContainersInBetween = clonedEssentials.filter(
        (cloned) => {
  ...
  _shouldShowTab(tab, workspaceUuid, containerId, workspaces) {
    ...
    if (isEssential) {
      ...
      return (
        ...
        !workspaces.some(
          (workspace) =>
            workspace.containerTabId === parseInt(tabContextId, 10),
        )
      );
    }
  ...
  isBookmarkInAnotherWorkspace(bookmark) {
    ...
    const activeWorkspaceUuid = this.getActiveWorkspaceFromCache()?.uuid;
    if (!activeWorkspaceUuid) {
      return false;
    }

    const validWorkspaceIds = new Set(
      this.getWorkspaces().map((workspace) => workspace.uuid),
    );
    ...
    for (const [workspaceUuid, bookmarkGuids] of Object.entries(
      ...
      if (
        !validWorkspaceIds.has(workspaceUuid) ||
        !Array.isArray(bookmarkGuids) ||
        !bookmarkGuids.includes(bookmarkGuid)
      ) {
        continue;
      }
